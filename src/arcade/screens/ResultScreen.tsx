        ...
        <div className="mt-2 text-sm text-ink-dim">
          {isRecord
            ? outcome.previousBest > 0
              ? t('result.previousBest', {
                  score: outcome.previousBest,
                  unit: game.scoreUnit,
                })
              : t('result.firstRecord')
            : t('result.bestRecord', {
                score: Math.max(outcome.previousBest, result.score),
                unit: game.scoreUnit,
              })}
        </div>
      </div>
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-bold text-white">
              {t('result.rankEntered', { rank: outcome.rank ?? 0 })}
            </p>
            <p className="text-[11px] text-ink-mute">{t('result.initialsHint')}</p>
          </div>
          <input
            ref={inputRef}
            value={initials}
